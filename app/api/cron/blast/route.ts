...
function c(v?:string){
  return v?v.trim().replace(/^["']|["']$/g,'').replace(/\n/g,'').replace(/\r/g,'').trim():'';
}
...
function o(h:string){
  ...
  const m=l.match(/copyright[^0-9]{0,10}(200[1-9]|201[0-9]|2020)/);
  if(m){s+=2;r.push(m[1]);}
...
async function g(k:string){
  try{
    const u1=c(process.env.KV_REST_API_URL),t1=c(process.env.KV_REST_API_TOKEN);
    const u2=c(process.env.UPSTASH_REDIS_REST_URL),t2=c(process.env.UPSTASH_REDIS_REST_TOKEN);
    let v1:any=null,v2:any=null;
    if(u1&&t1){try{const r=await fetch(`${u1}/get/${k}`,{headers:{Authorization:`Bearer ${t1}`}});const j=await r.json();if(j.result){let v=j.result;try{v=JSON.parse(v)}catch{};try{v=JSON.parse(v)}catch{};v1=v;}}catch{}}
    if(u2&&t2){try{const r=await fetch(`${u2}/get/${k}`,{headers:{Authorization:`Bearer ${t2}`}});const j=await r.json();if(j.result){let v=j.result;try{v=JSON.parse(v)}catch{};try{v=JSON.parse(v)}catch{};v2=v;}}catch{}}
    if(k==='current_state_index'){const n1=Number(v1||0),n2=Number(v2||0);return String(Math.max(n1,n2,n1||0,n2||0));}
...
async function w(k:string,v:any){
  ...
  try{
    const u1=c(process.env.KV_REST_API_URL),t1=c(process.env.KV_REST_API_TOKEN);
    const u2=c(process.env.UPSTASH_REDIS_REST_URL),t2=c(process.env.UPSTASH_REDIS_REST_TOKEN);
    if(u1&&t1){try{const r=await fetch(`${u1}/set/${k}`,{method:'POST',headers:{Authorization:`Bearer ${t1}`},body:JSON.stringify(JSON.stringify(v))});if(r.ok)ok=true;}catch{}}
    if(u2&&t2){try{const r=await fetch(`${u2}/set/${k}`,{method:'POST',headers:{Authorization:`Bearer ${t2}`},body:JSON.stringify(JSON.stringify(v))});if(r.ok)ok=true;}catch{}}
  }catch{}return ok;
}
...
const N:any={
  roofing:{q:(s:string)=>[`best roofing company ${s} site:.com`,`roofing contractor ${s} site:.com`],m:['roof'],t:'Roofing',l:[{n:'AI Roof Quote Estimator',d:'Instant quote'},{n:'Leak Scanner AI',d:'Photo leak'},{n:'Missed-Call Text-Back AI',d:'Never lose job'},{n:'Review Auto-Responder AI',d:'Auto replies'},{n:'Venus OS CRM',d:'Leads dashboard'}]},
  plumber:{q:(s:string)=>[`best plumber ${s} site:.com`,`plumbing company ${s} site:.com`],m:['plumb'],t:'Plumbing',l:[{n:'Emergency AI Dispatch',d:'24/7 booking'},{n:'Leak Price Calculator AI',d:'Instant price'},{n:'Missed-Call Text-Back AI',d:'Recovers calls'},{n:'Review Booster AI',d:'5-stars'},{n:'Venus OS Jobs CRM',d:'Dispatch'}]},
  hvac:{q:(s:string)=>[`best HVAC company ${s} site:.com`,`AC repair ${s} site:.com`],m:['hvac','air','heat','cool','ac'],t:'HVAC',l:[{n:'AC Repair Quote AI',d:'By model'},{n:'Duct Cost Estimator AI',d:'Duct pricing'},{n:'Missed-Call AI Closer',d:'After hours'},{n:'Google Review AI',d:'Auto response'},{n:'Venus OS Scheduler',d:'Scheduling'}]},
  electrical:{q:(s:string)=>[`best electrician ${s} site:.com`,`electrical contractor ${s} site:.com`],m:['electr'],t:'Electrical',l:[{n:'Electrical Quote AI',d:'Panel quotes'},{n:'Panel Upgrade Calculator',d:'Upgrade cost'},{n:'Emergency Call AI',d:'24/7'},{n:'Review Engine AI',d:'Reputation'},{n:'Venus OS Invoicing',d:'Leads to cash'}]},
  dentist:{q:(s:string)=>[`best dentist ${s} site:.com`,`dental office ${s} site:.com`],m:['dental','dentist','smile'],t:'Dental',l:[{n:'Smile Scan AI Booking',d:'From selfie'},{n:'Implant Price Estimator',d:'Pricing'},{n:'No-Show Rescue AI',d:'No-shows'},{n:'Review Growth AI',d:'5-stars'},{n:'Venus OS Patient CRM',d:'Patient mgmt'}]}
};
...
export async function GET(){
  ...
  try{
    ...
    if(lk){const age=Date.now()-Number(lk);if(age<90000)return new Response(JSON.stringify({ok:false,cur:curS,msg:`LOCKED ${90-Math.floor(age/1000)}s`,i}),{status:200});else await d('blast_lock');}
    await w('current_state_index',nxt);await w('blast_lock',Date.now());
    const all:any[]=[];for(const k of Object.keys(N)){const cfg:any=(N as any)[k];for(const q of cfg.q(curS)){try{const u=`https://serpapi.com/search.json?engine=google&q=${encodeURIComponent(q)}&num=10&api_key=${SK}`;const r=await fetch(u);const d=await r.json();for(const rr of (d.organic_results||[])){try{const dm=new URL((rr as any).link).hostname.replace('www.','').toLowerCase();if(dm)all.push({domain:dm,niche:k});}catch{}}}catch{}}}
    const byO:any={roofing:[],plumber:[],hvac:[],electrical:[],dentist:[]};const se=new Set<string>();
    for(const it of all as any[]){
      const d=(it.domain as string).toLowerCase();if(sS.has(d)||se.has(d)||J.some(j=>d.includes(j)))continue;se.add(d);
      const cfg:any=(N as any)[it.niche];if(!cfg.m.some((m:string)=>d.includes(m)))continue;
      try{const ac=new AbortController();const tm=setTimeout(()=>ac.abort(),2000);const pr=await fetch(`https://${d}`,{signal:ac.signal,headers:{'User-Agent':'Mozilla/5.0'}});clearTimeout(tm);if(!pr.ok)continue;const h=await pr.text();const a=o(h);if(a.isOld)byO[it.niche].push({domain:d,niche:it.niche,email:`info@${d}`,reason:a.reason,score:a.score,state:curS});M.push({domain:d,niche:it.niche,score:a.score,state:curS});}catch{}
    }
    const raw=[byO.roofing[0],byO.plumber[0],byO.hvac[0],byO.electrical[0],byO.dentist[0]].filter(Boolean);const mp=new Map();for(const c of raw as any[]){const k=(c.domain as string).toLowerCase();if(!mp.has(k))mp.set(k,c);}let toSend=Array.from(mp.values()).slice(0,5);
    ...
    await d('blast_lock');return new Response(JSON.stringify({ok:true,curS,nxtS,i,nxt,tot,R,msg:`V31 DUAL-KV: Mined ${curS} -> next ${nxtS}. Max index fix, PA never stuck.`}),{status:200});
