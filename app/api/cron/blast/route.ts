...
async function g(k:string){
 try{
  ...
  if(!u||!t)return{val:null,url:u||'no-url',token:!!t,err:'no env'};
  const r=await fetch(`${u}/get/${k}`,{headers:{Authorization:`Bearer ${t}`}});
  const j=await r.json();
  if(!j.result)return{val:null,url:u.substring(0,40),hasToken:true,raw:j};
  let v=j.result;try{v=JSON.parse(v);}catch{};try{v=JSON.parse(v);}catch{};return{val:v,url:u.substring(0,40),raw:j.result?.substring(0,100)};
 }catch(e:any){return{val:null,err:e.message};}
}
...
export async function GET(){
 ...
 try{
  const idxRaw:any=await g('current_state_index');
  const sentRaw:any=await g('sent_emails');
  const lockRaw:any=await g('blast_lock');
  let sA=idxRaw?.val;let arr=Array.isArray(sentRaw?.val)?sentRaw.val:(typeof sentRaw?.val==='string'?JSON.parse(sentRaw.val):sentRaw?.val||[]);
  let sS=new Set<string>((arr||[]).map((d:any)=>String(d).toLowerCase()));
  let i=sA;if(i===null||i===""||typeof i==='object')i=0;i=Number(i);
  const curS=S[i%S.length],nxt=(i+1)%S.length,nxtS=S[nxt];
  if(lockRaw?.val&&Date.now()-Number(lockRaw.val)<60000)return new Response(JSON.stringify({ok:false,cur:curS,msg:`LOCKED 60s`,idxRaw,sentCount:sS.size,lockRaw,url:idxRaw?.url}),{status:200});
  const lockOk=await w('blast_lock',Date.now(),true);
  if(!lockOk)return new Response(JSON.stringify({ok:false,cur:curS,msg:`LOCKED NX`,idxRaw,sentCount:sS.size,lockRaw}),{status:200});
  const all:any[]=[];
  for(const k of Object.keys(N)){
   ...
   for(const q of cfg.q(curS)){
    try{
     ...
     for(const rr of (d.organic_results||[])){try{const dm=new URL((rr as any).link).hostname.replace('www.','').toLowerCase();if(dm)all.push({domain:dm,niche:k});}catch{}}
    }catch{}
   }
  }
  ...
  for(const it of all as any[]){
   ...
   if(B.has(d))continue;if(sS.has(d))continue;if(J.some(j=>d.includes(j)))continue;if(se.has(d))continue;se.add(d);
   ...
   try{
    ...
    clearTimeout(tm);if(!pr.ok)continue;const h=await pr.text();const a=o(h);
  ...
  if(toSend.length<5){for(const m of M as any[]){if(toSend.length>=5)break;const k=(m.domain as string).toLowerCase();if(!mp.has(k)&&!sS.has(k)&&!B.has(k)){mp.set(k,{domain:m.domain,niche:m.niche,email:`info@${m.domain}`,reason:'popular '+m.state,score:m.score,state:curS});toSend=Array.from(mp.values()).slice(0,5);}}}
  if(toSend.length>=0){for(const c of toSend as any[])sS.add((c.domain as string).toLowerCase());await w('sent_emails',Array.from(sS));await w('current_state_index',nxt);}
  let tot=0;
  ...
  return new Response(JSON.stringify({ok:true,curS,nxtS,i,nxt,tot,R,idxRaw,sentCount:sS.size,url:idxRaw?.url,msg:`V29 DEBUG: Mined ${curS} -> next ${nxtS}. DB:${idxRaw?.url}`}),{status:200});
