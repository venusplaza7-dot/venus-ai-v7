...
<div style="max-width:600px;margin:0 auto">
  <div style="background:#fff;padding:24px;border-radius:12px;margin-bottom:16px;border:1px solid #e5e5e5">
    <p style="margin:0 0 12px 0;color:#111;font-size:14px;line-height:1.6">Hi, I'm from <b>Venus HQ</b> — we build AI websites for ${cat} in Houston.</p>
    <p style="margin:0 0 12px 0;color:#444;font-size:13px;line-height:1.6">I was auditing ${cat} sites in Houston and found <b>${oldDomain}</b> loads in 8s, no AI chat, no instant booking — you're losing 60% of leads to competitors who answer faster.</p>
    <p style="margin:0;color:#444;font-size:13px;line-height:1.6">So I built you a <b>live demo</b> already — Gen-Z luxury design + 4 AI agents that answer 24/7, book jobs, and capture calls. No pitch, just look:</p>
  </div>
  ...
  <div style="background:#fff;padding:20px;border-radius:12px;margin-top:16px;border:1px solid #e5e5e5">
    <p style="margin:0 0 8px 0;font-size:12px;font-weight:700;color:#111">WHAT WE OFFER:</p>
    <ul style="margin:0;padding-left:18px;color:#444;font-size:12px;line-height:1.8">
      ...
      <li><b>What we do:</b> Luxury site + AI Receptionist + Quote Bot + Review Bot + SEO — turns visitors to calls</li>
      <li><b>Price:</b> Original $1999, <b>Intro $497 one-time</b> — you own it, no monthly fee</li>
      <li><b>Next step:</b> Click View Live Proposal, if you like it we put your logo/number/city live in 24h</li>
    </ul>
    <p style="margin:16px 0 0 0;color:#111;font-size:13px">Want me to activate it for you?</p>
    <p style="margin:4px 0 0 0;color:#555;font-size:13px">— Venus Team • venushq7.com • noreply@venushq7.com</p>
  </div>
  <p style="text-align:center;color:#888;font-size:11px;margin-top:16px">One-time audit for your trade. Ignore if not interested — no more emails. <a href="https://venus-ai-v8.vercel.app/api/unsubscribe?email=${toEmail}" style="color:#888">Unsubscribe</a></p>
  <img src="https://venus-ai-v8.vercel.app/api/track?event=open&cat=${cat}" width="1" height="1"/>
</div>
</div>
`;
      const res = await fetch('https://api.brevo.com/v3/smtp/email',{method:'POST',headers:{'api-key':brevoKey,'Content-Type':'application/json'},body:JSON.stringify({sender:{name:'Venus HQ',email:'noreply@venushq7.com'},to:[{email:toEmail}],subject:`Your new ${cat} website is ready for review — $1999 → $497`,htmlContent:html})});
