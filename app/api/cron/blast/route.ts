...
const HARD_BLACKLIST = new Set([
  ...
  "houstonmedicalcenterdental.com",
  "bayouelectrical.com",
  "leethompsonac.com"
...
async function kvGet(): Promise<Set<string>> {
  try {
    ...
    const j=await res.json(); const arr=j.result? JSON.parse(j.result):[];
    return new Set(arr.map((d:string)=>String(d).toLowerCase().trim()));
  } catch { return new Set(); }
}
async function kvSet(set:Set<string>){
  try{
    const url=clean(process.env.KV_REST_API_URL)||clean(process.env.UPSTASH_REDIS_REST_URL)||clean(process.env.KV_URL);
    const token=clean(process.env.KV_REST_API_TOKEN)||clean(process.env.UPSTASH_REDIS_REST_TOKEN);
    const res=await fetch(`${url}/set/sent_emails`,{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify(JSON.stringify(Array.from(set)))});
    return res.ok;
  }catch{return false;}
}
...
export async function GET(){
  ...
  try{
    let sentSet=await kvGet();
    logs.push(`KV DIRECT loaded: ${sentSet.size} already sent - V23 with HARD_BLACKLIST of 9x spam 5`);
    ...
    logs.push(`MINED ${all.length} popular sites (150 target) - All popular sites not just roofing`);
    const byOld:Record<string,any[]>={roofing:[],plumber:[],hvac:[],electrical:[],dentist:[]};
    const byFall:Record<string,any[]>={roofing:[],plumber:[],hvac:[],electrical:[],dentist:[]};
    const seen=new Set<string>();
    for(const it of all){
      const d=it.domain.toLowerCase().trim();
      if(HARD_BLACKLIST.has(d)){ logs.push(`HARD_BLACKLIST SKIP ${d} - your 9x spam - NEVER AGAIN`); continue; }
      if(JUNK.some(j=>d.includes(j))) continue; if(d.endsWith('.ai')||d.endsWith('.io')) continue; if(d.length<8||d.length>35) continue; if(seen.has(d)) continue; seen.add(d);
    ...
    for(const c of toSend){
      ...
      try{
        ...
        const j=await res.json(); if(res.status===201){logs.push(`BREVO ${c.niche} 201 ${c.domain} OLD [${c.ageReason}] - V23 luxury white WHO/WHY/WHAT + HARD_BLACKLIST`); newlySent.push(c.domain); totalSent++;} else logs.push(`BREVO FAIL ${c.domain}: ${JSON.stringify(j)}`);
      }catch(e:any){logs.push(`BREVO ERR ${e.message}`);}
    }
    return new Response(JSON.stringify({ok:true,totalSent,alreadySentCount:sentSet.size-newlySent.length,newlySent,mined:mined.slice(0,15),logs,msg:`V23 FULL - Keeps ALL V17 luxury + HARD_BLACKLIST 9x spam 5 - Fixes 9x repeat - 150 popular, luxury white WHO/WHY/WHAT, never resend KV+HARD, only 2001-2020`}),{status:200});
