...
export async function POST(req:Request){
  ...
  try{
    ...
    return new Response(JSON.stringify({ok:true,cur:curS,curI:idx,nxt:nxtS,nxtI:nxtI,tot,allMined:all.length,keyDebug,dbg,msg:`V36 CORRECT KEY Mined ${all.length} -> Sent ${tot} from ${curS} key:${keyDebug} err:${dbg.serpError||'none'}`}),{headers:{'Content-Type':'application/json'}});
