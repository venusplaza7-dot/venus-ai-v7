...
export async function GET(){
  ...
  try{
    ...
    logs.push(`KV DIRECT loaded: ${sentSet.size} already sent - these WILL NEVER BE RESENT - fixes your 6x bug`);
    ...
    for(const it of all){
      const d=it.domain.toLowerCase().trim(); if(JUNK.some(j=>d.includes(j))) continue; if(d.endsWith('.ai')||d.endsWith('.io')) continue; if(d.length<8||d.length>35) continue; if(seen.has(d)) continue; seen.add(d);
      const cfg=NICHE_CONFIG[it.niche]; if(!cfg.must.some(m=>d.includes(m))) continue;
      if(sentSet.has(d)){ logs.push(`SKIP KV already sent ${d} - prevents 6x -> 7x`); continue; }
      try{
    ...
    logs.push(`FINAL: R${final.roofing.length} P${final.plumber.length} H${final.hvac.length} E${final.electrical.length} D${final.dentist.length} -> SEND ${toSend.length} BALANCED (not 2 only)`);

    // LOCK BEFORE SEND - DIRECT REST - fixes quotes
    if(toSend.length>0){
      for(const c of toSend) sentSet.add(c.domain.toLowerCase());
      const ok=await kvSet(sentSet);
      logs.push(ok?`KV DIRECT LOCKED BEFORE SEND: ${sentSet.size} total - ${toSend.map((t:any)=>t.domain).join(', ')} - WILL NEVER REPEAT 6x`:`KV SAVE FAIL - check Upstash env vars have no quotes`);
    }
    let totalSent=0;
    for(const c of toSend){
      ...
      try{
        ...
        const j=await res.json(); if(res.status===201){logs.push(`BREVO ${c.niche} 201 ${c.domain} OLD [${c.ageReason}] - luxury white WHO/WHY/WHAT`); newlySent.push(c.domain); totalSent++;} else logs.push(`BREVO FAIL ${c.domain}: ${JSON.stringify(j)}`);
      }catch(e:any){logs.push(`BREVO ERR ${e.message}`);}
    }
    return new Response(JSON.stringify({ok:true,totalSent,alreadySentCount:sentSet.size-newlySent.length,newlySent,mined:mined.slice(0,15),logs,msg:`V22 DIRECT REST - Fixes 6x repeat - Sent ${totalSent} balanced - Includes all 4 from screenshot: 150 popular, luxury white WHO/WHY/WHAT, never resend KV direct, only 2001-2020`}),{status:200});
