...
export async function GET(){
 ...
 try{
  ...
  for(const it of all as any[]){
   ...
   if(B.has(d))continue;if(sS.has(d))continue;
   if(J.some(j=>d.includes(j)))continue;if(se.has(d))continue;se.add(d);
   const cfg:any=(N as any)[it.niche];
   if(!cfg.m.some((m:string)=>d.includes(m)))continue;
   try{
    ...
    clearTimeout(tm);if(!pr.ok)continue;
    const h=await pr.text();const a=o(h);
    if(a.isOld){byO[it.niche].push({domain:d,niche:it.niche,email:`info@${d}`,reason:a.reason,score:a.score,state:curS});}
    M.push({domain:d,niche:it.niche,score:a.score,state:curS});
   }catch{}
  }
  ...
  if(toSend.length>=0){
   ...
   await w('sent_emails',Array.from(sS));await w('current_state_index',nxt);
  }
  let tot=0;
