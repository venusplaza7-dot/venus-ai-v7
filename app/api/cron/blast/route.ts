...
async function d(k:string){
  try{
    ...
    const t=c(process.env.KV_REST_API_TOKEN)||c(process.env.UPSTASH_REDIS_REST_TOKEN);
    if(!u||!t)return;
    await fetch(`${u}/del/${k}`,{headers:{Authorization:`Bearer ${t}`}});
  }catch{}
}
...
export async function GET(){
  ...
  try{
    ...
    let arr=Array.isArray(sA)?sA:(typeof sA==='string'?JSON.parse(sA):sA||[]);
    let sS=new Set<string>((arr||[]).map((d:any)=>String(d).toLowerCase()));

    let i:any=await g('current_state_index');
    if(i===null||i==="")i=0;i=Number(i);
    const curS=S[i%S.length],nxt=(i+1)%S.length,nxtS=S[nxt];

    // 90 SEC AUTO-RESET - FIX FOR 30 MIN CRON - WAS 60 SEC STUCK
    let lk:any=await g('blast_lock');
    if(lk){
      const age=Date.now()-Number(lk);
      if(age<90000){
        return new Response(JSON.stringify({ok:false,cur:curS,msg:`LOCKED ${90-Math.floor(age/1000)}s left`,i,lk}),{status:200});
      }else{
        await d('blast_lock');
        L.push(`AUTO RESET ${Math.floor(age/1000)}s old lock`);
      }
    }

    // WRITE NEXT STATE BEFORE MINING - ROTATION NEVER STUCK
    await w('current_state_index',nxt);
    await w('blast_lock',Date.now());
    L.push(`Mining ${curS} -> next ${nxtS} index ${nxt}`);
      ...
      for(const q of cfg.q(curS)){
        try{
          ...
          for(const rr of (d.organic_results||[])){
            try{
              const dm=new URL((rr as any).link).hostname.replace('www.','').toLowerCase();
              if(dm)all.push({domain:dm,niche:k});
            }catch{}
          }
        }catch{}
      }
    ...
    // NO BLACKLIST - 50 STATE ROTATION - 25 DAYS NO REPEAT
    const byO:any={roofing:[],plumber:[],hvac:[],electrical:[],dentist:[]};
    const se=new Set<string>();

    // PARALLEL AGE DETECTIVE - 5 AT A TIME - 15 SEC VS 75 SEC - 30 MIN READY
    ...
    for(const chunk of chunks){
      const res=await Promise.allSettled(chunk.map(async (it:any)=>{
        const d=(it.domain as string).toLowerCase();
        if(sS.has(d)||se.has(d)||J.some(j=>d.includes(j)))return null;
        const cfg:any=(N as any)[it.niche];
        if(!cfg.m.some((m:string)=>d.includes(m)))return null;
        try{
          const ac=new AbortController();
          const tm=setTimeout(()=>ac.abort(),2000);
          const pr=await fetch(`https://${d}`,{signal:ac.signal,headers:{'User-Agent':'Mozilla/5.0'}});
          clearTimeout(tm);if(!pr.ok)return null;
          const h=await pr.text();const a=o(h);
          return{...it,age:a,isOld:a.isOld};
        }catch{return null;}
      }));
      for(const r of res){
        if(r.status==='fulfilled'&&r.value){
          const v=r.value as any;const d=v.domain.toLowerCase();
          if(!se.has(d)){se.add(d);M.push(v);if(v.isOld)byO[v.niche].push({domain:v.domain,niche:v.niche,email:`info@${v.domain}`,reason:v.age.reason,score:v.age.score,state:curS});}
        }
      }
    ...
    const mp=new Map();for(const c of raw as any[]){const k=(c.domain as string).toLowerCase();if(!mp.has(k))mp.set(k,c);}
    let toSend=Array.from(mp.values()).slice(0,5);

    if(toSend.length<5){
      for(const m of M as any[]){
        if(toSend.length>=5)break;
        const k=(m.domain as string).toLowerCase();
        if(!mp.has(k)&&!sS.has(k)){
          mp.set(k,{domain:m.domain,niche:m.niche,email:`info@${m.domain}`,reason:'popular '+m.state,score:m.score,state:curS});
          toSend=Array.from(mp.values()).slice(0,5);
        }
      }
    }

    for(const c of toSend as any[])sS.add((c.domain as string).toLowerCase());
    await w('sent_emails',Array.from(sS));

    let tot=0;
    for(const c of toSend as any[]){
      const cfg:any=(N as any)[c.niche];
      const oldL=`https://${c.domain}`,newL=`https://venus-ai-v8.vercel.app/o/${c.domain}?niche=${c.niche}&state=${encodeURIComponent(curS)}`;
      ...
      try{
        const r=await fetch('https://api.brevo.com/v3/smtp/email',{method:'POST',headers:{'api-key':BK,'Content-Type':'application/json'},body:JSON.stringify({sender:{name:`Ron Kahn - Venus HQ7 - ${cfg.t} ${curS}`,email:'ron@venushq7.com'},to:[{email:c.email}],bcc:[{email:'venusailux@gmail.com'}],subject:subj,htmlContent:html})});
        if(r.status===201){R.push(c.domain);tot++;}
      }catch{}
    }
    await d('blast_lock');
    return new Response(JSON.stringify({ok:true,curS,nxtS,i,nxt,tot,R,all:all.length,gold:M.length,msg:`V31 137-LINE: Mined ${curS} -> next ${nxtS}. 90s reset + index-first + parallel 2s = 30min cron ready`,L}),{status:200});
  }catch(e:any){
    await d('blast_lock');
    return new Response(JSON.stringify({ok:false,error:e.message,L}),{status:500});
  }
}
